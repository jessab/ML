...
def applyFun(dic, fun, postfix):
    res = dict({k + "." + postfix: fun(dic[k]) for k in dic.keys() if
                dic[k] is not None})
    res.update(dict({k + "." + postfix: None for k in dic.keys()
                     if dic[k] is None}))
    return res
...
def posPeaks():
    ...
    withSmooth = [(dt, st, sc) for dt in ['simple', 'cwt']
                for st in ['hilbert', 'sg', 'butter'] for sc in [True, False]]
