...
def addFeatureToDict(dic, f):
    ...
    dic[bodyPart][cat]['cols'] += [c for c in cols
                        if c not in dic[bodyPart][cat]['cols']]
    dic[bodyPart][cat]['features'] += [f for f in features
                        if f not in dic[bodyPart][cat]['features']]
...
if __name__ == '__main__':
    features = ['hip.Ax.min', 'ankle.AyAz.fcovar',
                'hip.Vx.av', 'ankle.simple_nosmooth.maxDist',
                'hip.cwt_butter_ncor.avPeak',
