...
from sklearn import preprocessing as pp, svm, cross_validation, tree, datasets
...
def selektKBestUncorrelatedFeatures(samples, classifications,
...
def selectFeatures(samples, classifications, featureNames,
                   classifierClass, nbFeatures=10):
#     [samples,featureNames] = selectBestFeaturesRFECV(samples,
#                               classifications, featureNames, classifierClass)
#     [samples,featureNames] = selectKBestFeatures(samples,
#                               classifications, featureNames, nbFeatures)
    [samples, featureNames] = selektKBestUncorrelatedFeatures(samples,
                                classifications, featureNames, nbFeatures)
    return [samples, featureNames]
...
def classifyData(data, classifyTrained,
                 classifySurface, classifierClass, featuresSelect):
    ...
    if (featuresSelect):
        [samples, featureNames] = selectFeatures(samples, classifications,
                                            featureNames, classifierClass)
    classifier = classifierClass(samples,
                                 featureNames, classifications, classNames)

    return classifier


def classifyDataDT(data, classifyTrained,
                   classifySurface, selectFeatures=False):
...
def classifyDataSVM(data, classifyTrained,
                    classifySurface, selectFeatures=False):
    return classifyData(data, classifyTrained,
                        classifySurface, SVMClassifier, selectFeatures)


def classifyDataKNN(data, classifyTrained,
                    classifySurface, selectFeatures=False):
    return classifyData(data, classifyTrained,
                        classifySurface, KNNClassifier, selectFeatures)
...
class Classifier(object):
    ...
    def plotDecisionSurface(self):
        ...
        [samples, featureNames] = selectFeatures(self.samples,
                self.classifications, self.featureNames, classifierClass, 2)
        clf = classifierClass(samples, featureNames,
                            self.classifications, self.classNames)
        ...
        ttl = (self.classifierName() +
               "  " + featureNames[0] + " - " + featureNames[1])
...
#             print(" %i:\t%s" % (i, str(svector)))
        ...
        for i, fa in enumerate(svArray):
            if (i < len(self.classNames)):
                ...
                bestFeatures = heapq.nlargest(5,
                                enumerate(fa), operator.itemgetter(1))
...
class DTClassifier(Classifier):
    ...
    def __init__(self,
                 samples, featureNames, classifications, classificationNames):
...
class KNNClassifier(Classifier):
    def __init__(self, samples,
                 featureNames, classifications, classificationNames, k=5):
